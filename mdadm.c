...
int mdadm_mount(void) {
  ...
  if (mountStatus == 0) 
  {
    ...
    mountStatus = 1;
    jbod_client_operation(op(0,JBOD_MOUNT,0,0), NULL);
...
int mdadm_unmount(void) {
  ...
  if (mountStatus == 1)
  {
    ...
    mountStatus = 0;
    jbod_client_operation(op(0,JBOD_UNMOUNT,0,0), NULL);
...
int mdadm_read(uint32_t start_addr, uint32_t read_len, uint8_t *read_buf)  {
  ...
  else if (mountStatus == 1)
  {
    ...
    while (x < y)
    {
      ...
      uint8_t tempBuf[JBOD_BLOCK_SIZE] = {0};

      // Cache implementation for reading memory quicker than accessing main memory. If cache is not on, then else block statements execute.
      if (r == true)
      {
        int m = cache_lookup(diskNum, blockNum, tempBuf);
        ...
        {   
          if (((x + JBOD_BLOCK_SIZE) <= y)|| x == start_addr)
          {
            ...
            if (read_len <= currentBlockLeft)
            {
              ...
              memcpy(read_buf + remainderBits, tempBuf + addrOffset, read_len);
              break;

            }

            memcpy(read_buf + remainderBits, tempBuf + addrOffset, currentBlockLeft);
          ...
          else if ((x + JBOD_BLOCK_SIZE) >= y)
          {
            ...
            memcpy(read_buf + remainderBits, tempBuf, read_len - remainderBits);
            break;

          } 

        } 
        // Lookup fails: block not found in cache so main memory needs to be read and block inserted into cache.
        else if (m == -1)
        {
          
          jbod_client_operation(op(0,JBOD_SEEK_TO_DISK,0,diskNum), NULL);
          jbod_client_operation(op(0,JBOD_SEEK_TO_BLOCK,blockNum,0), NULL);
          jbod_client_operation(op(0,JBOD_READ_BLOCK,0,0), tempBuf);
          ...
          if (((x + JBOD_BLOCK_SIZE) <= y)|| x == start_addr)
          {
            ...
            if (read_len <= currentBlockLeft)
            {
              ...
              memcpy(read_buf, tempBuf + addrOffset, read_len);
              break;
      ...
      else
      {
        ...
        jbod_client_operation(op(0,JBOD_SEEK_TO_DISK,0,diskNum), NULL);
        jbod_client_operation(op(0,JBOD_SEEK_TO_BLOCK,blockNum,0), NULL);
        jbod_client_operation(op(0,JBOD_READ_BLOCK,0,0), store);
...
int mdadm_write(uint32_t start_addr, uint32_t write_len, const uint8_t *write_buf) {
  ...
  else if (mountStatus == 1)
  {
    ...
    while (x < y)
    {
      ...
      int currentBlockLeft = (uint32_t) (JBOD_BLOCK_SIZE - addrOffset);
      ...
      if (r == true)
      {
        ...
        if (m == 1)
        {
          ...
          memcpy(tempBuf + addrOffset, write_buf + remainderBits, currentBlockLeft);

          jbod_client_operation(op(0,JBOD_SEEK_TO_DISK,0,diskNum), NULL);
          jbod_client_operation(op(0,JBOD_SEEK_TO_BLOCK,blockNum,0), NULL);
          jbod_client_operation(op(0,JBOD_WRITE_BLOCK,0,0), tempBuf);
      ...
      else
      {
        ...
        if (((x + (uint32_t) JBOD_BLOCK_SIZE) <= y)|| x == start_addr)
        {
          ...
          if (write_len <= currentBlockLeft)
          {
            ...
            jbod_client_operation(op(0,JBOD_SEEK_TO_DISK,0,diskNum), NULL);
            jbod_client_operation(op(0,JBOD_SEEK_TO_BLOCK,blockNum,0), NULL);
            jbod_client_operation(op(0,JBOD_WRITE_BLOCK,0,0), store);
